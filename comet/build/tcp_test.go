...
func (*MyCodec) Decode(c gnet.Conn) ([]byte, error) {
	...
	bodyLen := int(common.BytesToInt32(buf))
	c.ShiftN(HeadSize)
...
func (*echoDemo) EventType() comet.NetWorkEventType {
	return comet.NetWorkEventReact
}
...
func TestClient(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		b := []byte("hello")
		buffer := bytes.NewBuffer([]byte{})
		buffer.Write(common.Int32ToBytes(int32(len(b))))
		buffer.Write(b)
