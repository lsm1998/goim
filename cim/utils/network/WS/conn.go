...
// new connection
func newWSConn(rwc io.ReadWriteCloser, r *bufio2.Reader, w *bufio2.Writer, remoteAddr string) *WSConn {
	return &WSConn{rwc: rwc, r: r, w: w, maskKey: make([]byte, 4), remoteAddr: remoteAddr}
}

// WriteHeader write header frame.
func (c *WSConn) WriteHeader(msgType int, length int) (err error) {
	var h []byte
...
func (c *WSConn) WriteBody(b []byte) (err error) {
	if len(b) > 0 {
		_, err = c.w.Write(b)
	}
	return
...
// Peek write peek.
func (c *WSConn) Peek(n int) ([]byte, error) {
	return c.w.Peek(n)
}
...
func (c *WSConn) WriteMessage(msgType int, msg []byte) (err error) {
...
func (c *WSConn) ReadMessage() (op int, payload []byte, err error) {
	var (
		fin         bool
		finOp, n    int
		partPayload []byte
...
func (c *WSConn) readFrame() (fin bool, op int, payload []byte, err error) {
	var (
		b          byte
		p          []byte
		mask       bool
...
func (c *WSConn) Context() (ctx interface{}) {
	return nil
}

func (c *WSConn) SetContext(ctx interface{}) {
}

func (c *WSConn) LocalAddr() (addr net.Addr) {
	return nil
}

func (c *WSConn) RemoteAddr() (addr net.Addr) {
	_, ip, err := net.ParseCIDR(c.remoteAddr)
	if err != nil {
		return nil
	}
	return ip
...
func (c *WSConn) BufferLength() (size int) {
	return 0
}

func (c *WSConn) SendTo(buf []byte) error {
	return nil
}

func (c *WSConn) AsyncWrite(buf []byte) error {
	if err := c.WriteMessage(BinaryMessage, buf); err != nil {
		return err
	}
	return c.Flush()
}

func (c *WSConn) Wake() error {
	return nil
}
